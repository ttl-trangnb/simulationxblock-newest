...
class OfficeQuestionBankXBlock(XBlock, CompletableXBlockMixin):
    ...
    def get_state_definitions(self):
        if not self.simulation_content_json_path:
            log.info("simulation_content_json_path is None or empty.")
        ...
        if len(self.simulation_content_json_path) > 0:
            ...
            if not file_content:
                log.warning("No content retrieved from S3 for path: %s", json_path)
                return self.state_definitions
